...
//   - 定义一个 `Map` 集合, 并将 Map 的 Value 类型定义为 `struct{}`
//   - 将 `Map` 集合的 Key 作为 Set 集合的元素
//
// `struct{}` 相当于一个空类型, 不占用实际的存储空间
...
// 创建并初始化 Set 集合对象
//
// 通过 `cap` 参数指定 Set 集合的初始容量
func New[T any](cap int) *Set[T] {
	return &Set[T]{
		m:   make(map[any]struct{}, cap),
		cap: cap,
	}
}
...
func (s *Set[T]) IsSubset(other *Set[T]) bool {
	...
	for v := range s.m {
		if !other.Contains(v.(T)) { // 另一个集合是否包含 当前集合的 所有元素
			return false
		}
	}
	return true
