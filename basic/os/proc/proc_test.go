...
func TestOS_StartProcess(t *testing.T) {
	...
	// 启动进程
	// 注意, 进程参数的第一项必须是进程名称
	p, err := os.StartProcess(cmd, []string{"ls", "-lh"}, &os.ProcAttr{
		Files: []*os.File{os.Stdin, pi.Writer(), os.Stderr},
	})
	assert.Nil(t, err)
	...
	// 系统执行占用的用户 CPU 时间
	assert.Greater(t, fi.UserTime().Nanoseconds(), int64(0))
...
func TestOS_Process_Kill(t *testing.T) {
	...
	p, err := os.StartProcess(cmd, []string{"sleep", "10s"}, &os.ProcAttr{Files: []*os.File{os.Stdin, os.Stdout, os.Stderr}})
	assert.Nil(t, err)
