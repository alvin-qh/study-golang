...
func TestTimeAfter(t *testing.T) {
	...
	c := time.After(3 * time.Second) // 3 秒后发送信号
...
func TestTimeAfterFunc(t *testing.T) {
	...
	time.AfterFunc(time.Second*3, func() { // 3 秒后回调函数
		ch <- struct{}{} // 发送一个空的信号
	})
...
func TestTicker(t *testing.T) {
	...
	tk := time.NewTicker(1 * time.Second) // 每隔 1 秒发送一次信号
	defer tk.Stop()
	...
	for n < 2 { // 获取两次信号
		...
		assert.Equal(t, n+1, int(d.Seconds()))
...
func TestTimer(t *testing.T) {
	...
	ti := time.NewTimer(2 * time.Second) // 2 秒后发送定时器信号
	defer ti.Stop()
