...
func TestGetCallStack(t *testing.T) {
	...
	assert.Equal(t, `basic/runtime/callstack.CallStack()`, lines[1])                                          // 第 2 行为当前调用函数，形如：basic/runtime/callstack.CallStack()
	...
	assert.Regexp(t, `basic/runtime/callstack.TestGetCallStack\(0x[a-zA-Z0-9]+\)`, lines[3])
	...
	defer func() { // 处理 panic 的 defer 调用
		...
		assert.Regexp(t, `basic/runtime/callstack.TestGetCallStack.func2()`, lines[3])
