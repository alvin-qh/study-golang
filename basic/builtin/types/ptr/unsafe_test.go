...
func TestUnsafe_PointerMovement(t *testing.T) {
	...
	t.Run("access int64 variable by uint32 pointer", func(t *testing.T) {
		...
		s1 := (*uint16)(pn)
		// 相当于取 n 变量的第 3~4 字节
		s2 := PtrAdd((*uint16)(pn), 2)
		// 相当于取 n 变量的第 5~6 字节
		s3 := PtrAdd((*uint16)(pn), 4)
		// 相当于取 n 变量的第 7~8 字节
		s4 := PtrAdd((*uint16)(pn), 6)
	...
	t.Run("access int64 variable by uint8 pointer", func(t *testing.T) {
		...
		b1 := (*uint8)(pn)
		// 相当于取 n 变量第 2 字节
		b2 := PtrAdd((*uint8)(pn), 1)
		// 相当于取 n 变量第 3 字节
		b3 := PtrAdd((*uint8)(pn), 2)
		// 相当于取 n 变量第 4 字节
		b4 := PtrAdd((*uint8)(pn), 3)
		// 相当于取 n 变量第 5 字节
		b5 := PtrAdd((*uint8)(pn), 4)
		// 相当于取 n 变量第 6 字节
		b6 := PtrAdd((*uint8)(pn), 5)
		// 相当于取 n 变量第 7 字节
		b7 := PtrAdd((*uint8)(pn), 6)
		// 相当于取 n 变量第 8 字节
		b8 := PtrAdd((*uint8)(pn), 7)
...
func TestUnsafe_PointerOfStruct(t *testing.T) {
	...
	assert.Equal(t, int8(1), *PtrAdd((*int8)(pv), offs["I1"]))
	assert.Equal(t, int16(2), *PtrAdd((*int16)(pv), offs["I2"]))
	assert.Equal(t, int32(3), *PtrAdd((*int32)(pv), offs["I3"]))
	assert.Equal(t, int64(4), *PtrAdd((*int64)(pv), offs["I4"]))
	assert.Equal(t, []rune{'a', 'b', 'c'}, *PtrAdd((*[]rune)(pv), offs["A"]))
}
