...
// 零拷贝字符串转为字节切片
//
// 通过将字符串的数据头换为字节切片的数据头, 实现在不拷贝数据的前提下将字符串转为字节切片
//
// 在 Go 1.20 以上版本, 可以通过 `unsafe.StringData` 配合 `unsafe.Slice` 方法进行转换
//
//	s := "hello, 大家好"
//	bs := unsafe.Slice(unsafe.StringData(s), len(s)) // 将变量 s 就地转为 []byte
//	s = ""
...
// 零拷贝将字节切片转换为字符串
//
// 通过将切片的数据头换为字符串的数据头, 实现在不拷贝数据的前提下将字节切片转为字符串
//
// 在 Go 1.20 以上版本, 可以通过 `unsafe.SliceData` 配合 `unsafe.String` 方法进行转换
//
//	bs := []byte("hello, 大家好")
//	s := unsafe.String(unsafe.SliceData(bs), len(bs)) // 将变量 bs 就地转为 string 类型
//	bs = nil
func zeroCopyBytesToString(bs []byte) string {
	sh := (*reflect.SliceHeader)(unsafe.Pointer(&bs))

	var s string
	bh := (*reflect.StringHeader)(unsafe.Pointer(&s))
	bh.Data = sh.Data
	bh.Len = sh.Len
...
func (b *BufferIO) ReadString(size int) (string, error) {
	...
	zeroCopyBytesToString(bs)
	return zeroCopyBytesToString(bs), nil
}
